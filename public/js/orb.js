/**
 * Orb WebGL Effect — Vanilla JS (converted from React/OGL)
 * A mesmerizing animated orb with hover interactions using raw WebGL
 */
...
(function () {
  ...
  function initOrb(container, options) {
    ...
    const uBgColor = gl.getUniformLocation(program, 'backgroundColor');
    ...
    let lastTime = 0;
    let rafId;
    const bgRGB = hexToRGB(opts.backgroundColor);

    // Resize handler
    function resize() {
      const dpr = Math.min(window.devicePixelRatio || 1, 1); // Cap DPR to 1 for performance
    ...
    function frame(t) {
      ...
      gl.uniform1f(uHue, opts.hue);
      gl.uniform1f(uHover, currentHover);
      gl.uniform1f(uRot, currentRot);
      gl.uniform1f(uHoverIntensity, opts.hoverIntensity);
  ...
  // ─── Auto-init all orb containers ────────────────────────────────
  document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.orb-wrapper').forEach(function (el) {
      initOrb(el, {
        hue: parseFloat(el.dataset.hue || 0),
        hoverIntensity: parseFloat(el.dataset.hoverIntensity || 2),
        rotateOnHover: el.dataset.rotateOnHover !== 'false',
        forceHoverState: el.dataset.forceHover === 'true',
        backgroundColor: el.dataset.bgColor || '#000000'
      });
    });
  });
})();
